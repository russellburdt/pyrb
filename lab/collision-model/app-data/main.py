...
def events_selected_callback(attr, old, new):
    ...
    # html video tag
    path = os.path.join('http://10.144.240.35:5007/app-data', 'static', os.path.split(dst)[1])
    video_events.text = f"""
...
datadir = r'/mnt/home/russell.burdt/data/collision-model/great-west'
...
assert os.path.isfile(os.path.join(datadir, 'df-app.p'))

# read data
dcm = pd.read_pickle(os.path.join(datadir, 'dcm-gps.p'))
dfa = pd.read_pickle(os.path.join(datadir, 'df-app.p'))
...
eval_select = column(
    vs['main-title'], vs['industry-select'], vs['company-select'], vs['vehicle-select'], vs['window-select'],
    vs['filter-title'], vs['filter-radio'], vs['filter'], status)
layout_gps = row(
    eval_select,
