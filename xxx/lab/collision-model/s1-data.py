...
# lytx.parquet_events_to_behaviors(path=datadir, spark=spark, dataset='behaviors', nv=10000)
...
# df = dbv[['VehicleId', 'ta', 'tc']].rename(columns={'ta': 't0', 'tc': 't1'})
# lytx.distributed_data_extraction(datadir=datadir, dataset='dce_scores', df=df, nx=300, nd=60, spark=spark, distributed=False)
...
# # gpsymd parquet dataset
# gps = spark.read.parquet(os.path.join(datadir, 'gpsymd.parquet'))
# gps.createOrReplaceTempView('gps')
# vids = spark.sql(f'SELECT DISTINCT(VehicleId) FROM gps').toPandas().values.flatten()
# v0, c0 = vids.size, gps.count()
# print(v0, c0)

# # gpsymd to gps parquet dataset, batches of nx vehicles
# nx = 6000
# rows = int(np.ceil(vids.size / nx))
# vxs = np.hstack((vids, np.tile(None, rows * nx - vids.size))).reshape(rows, nx)
# for vx in tqdm(vxs, desc='gpsymd to gps'):
#     vx = np.array([x for x in vx if x is not None])
#     vs = ','.join([f"""'{x}'""" for x in vx])
#     sdf = spark.sql(f"""SELECT * FROM gps WHERE VehicleId IN ({vs})""")
#     sdf.write.parquet(path=os.path.join(datadir, 'gps.parquet'), partitionBy='VehicleId', mode='append')
# gps = spark.read.parquet(os.path.join(datadir, 'gps.parquet'))
# gps.createOrReplaceTempView('gps')
# v1, c1 = spark.sql(f'SELECT COUNT(DISTINCT(VehicleId)) FROM gps').toPandas().values[0][0], gps.count()
# assert (v0 == v1) and (c0 == c1)

# merge and remove duplicates from gps parquet dataset
