...
def cpm_artifacts(population, min_days_segments=None):
    """
    artifacts for collision prediction model dataset
    """

    # datadir and metadata
    datadir = rf'/mnt/home/russell.burdt/data/collision-model/{population}'
    assert os.path.isdir(datadir)
    dcm = pd.read_pickle(os.path.join(datadir, 'population-dataframe.p'))
    dm = pd.read_pickle(os.path.join(datadir, 'metrics.p'))
    ds = pd.read_pickle(os.path.join(datadir, 'gps_segmentation_metrics.p'))

    # validate
    x0 = dm.loc[~pd.isnull(dm['gps_miles'])].index
    x1 = ds.loc[ds['n_records_segments'] > 1].index
    assert (x0.size == x1.size) and all(x0 == x1)
    assert (dcm.shape[0] > dm.shape[0]) and (dcm.shape[0] > ds.shape[0])

    # filter dataset by n_days_segments
    if min_days_segments is not None:
        assert isinstance(min_days_segments, int)
        ok = ds.loc[ds['n_days_segments'] > min_days_segments].index.to_numpy()
        dcm, dm, ds = dcm.loc[ok].reset_index(drop=True), dm.loc[ok].reset_index(drop=True), ds.loc[ok].reset_index(drop=True)
    else:
        dm = pd.merge(left=pd.DataFrame(index=dcm.index), right=dm, left_index=True, right_index=True, how='left')
        ds = pd.merge(left=pd.DataFrame(index=dcm.index), right=ds, left_index=True, right_index=True, how='left')

    # validate and return
    assert (dcm.shape[0] == dm.shape[0] == ds.shape[0]) and all(dcm.index == dm.index) and all(dcm.index == ds.index)
    assert len(set(dcm.columns).intersection(dm.columns)) == 0
    return dcm, dm
