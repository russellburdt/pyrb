...
"""
utils for collision prediction model
"""
...
from lytx import get_conn, time_bounds_dataframe
...
# modify dcm for distributed data extraction
...
def expanding_bar_chart(x, labels, title, xlabel, legend, figsize=(8, 3), size=8, height=0.8):
    """
    ...
    ** currently supports 1 or 2 labels only **
    """

    def get_existing_data_and_reset():
        """
        - return existing bar widths (x0), ticks (y0), and legends (leg0)
        - close and reset existing fig / ax objects
        """
        x0, y0 = [], []
        for container in ax.containers:
            x0.append([x.get_width() for x in container])
            y0.append([x.get_y() + x.get_height() / 2 for x in container])
        x0, y0 = np.array(x0), np.array(y0)
        leg0 = [x.get_text() for x in ax.get_legend().texts]
        plt.close(fig)
        a, b = open_figure(title, figsize=figsize)
        return x0, y0, leg0, a, b
    ...
    lsame = None
    if ax.containers:
        l0 = np.array([x.get_text() for x in ax.get_yticklabels()])
        if (l0.size == labels.size) and (all(l0 == labels)):
            lsame = True
        elif len(set(l0).intersection(labels)) == 0:
            lsame = False
        else:
    ...
    # existing data and all same labels
    elif isinstance(lsame, bool) and lsame:
        x0, y0, leg0, fig, ax = get_existing_data_and_reset()
        xs = np.vstack((x0, x))
        height = height / xs.shape[0]
        ticks = np.tile(np.expand_dims(np.arange(xs.shape[1]), axis=1), xs.shape[0]).T.astype(np.float)
        legs = leg0 + [legend]
        ticks[0, :] -= 0.2
        ticks[1, :] += 0.2
        for x, y, leg in zip(xs, ticks, legs):
            ax.barh(y=y, width=x, height=height, align='center', label=leg)
        ax.set_yticks(range(xs.shape[1]))
        ax.set_ylim(-height * xs.shape[0], xs.shape[1] - 1 + height * xs.shape[0])
        ax.set_yticklabels(labels)

    # existing data and all different labels
    elif isinstance(lsame, bool) and not lsame:
        x0, y0, leg0, fig, ax = get_existing_data_and_reset()
        xs = [x0.flatten(), x]
        legs = leg0 + [legend]
        offset = 0
        for x, leg in zip(xs, legs):
            y = offset + np.arange(x.size)
            offset += x.size
            ax.barh(y=y, width=x, height=height, align='center', label=leg)
        ax.set_yticks(range(offset))
        ax.set_ylim(-height, offset - 1 + height)
        ax.set_yticklabels(np.hstack((l0, labels)))
