...
def merge_parquet(spark, loc, partition='VehicleId'):
    ...
    # func(pdf.loc[pdf.index == np.where(pd.DataFrame(data={'pxs': pxs})['pxs'].str.contains('9100FFFF-48A9-D663-DF82-8E63F0480000'))[0][0]])
    ...
    def func(year, month, day):
        ...
        query = f"""
            SELECT
                ...
                hdop, serialnumber, companyid, state_province, posted_speed, timezone, numsats, horizontalaccuracy
            FROM gps_prod
...
def query_gps_direct(df, dt, path, spark):
    """
    gps query based on df and dt, store as Parquet dataset at path, needs below
    - direct query via spark to s3
    - did not scale
...
def query_erf(df, dt, path, spark):
    """
    query event-recorder-files based on df and dt, store as Parquet dataset at path (in-progress)
    """
...
def gps_test_etl_pattern(spark, datadir, src, dst, service):
    ...
    vx = spark.range(start=0, end=vids.shape[0], step=1, numPartitions=vids.shape[0])
    dx = vx.groupby('id').applyInPandas(gps_test_etl_pattern_vid, schema=vx.schema).toPandas()
    # debug
    # vx = vx.toPandas()
    # gps_test_etl_pattern_vid(vx.loc[vx.index == 0])
    # gps_test_etl_pattern_vid(vx.loc[vids['VehicleId'] == ''])
...
def spark_etl_load_gps(datadir, src, vid, service):
    ...
    assert fn[-8:] == '.parquet'
    df = pq.ParquetFile(fn).read().to_pandas().sort_values('epoch').reset_index(drop=True)
    df['VehicleId'] = vid
    return df
...
def gps_segmentation(spark, datadir, src, dst, service, time_interval_sec=61, distance_interval_miles=1.0, mph_max=200):
    ...
    def gps_segmentation_vid(pdf):
        ...
        # load gps data, validate, remove partial duplicates
        df = spark_etl_load_gps(datadir, src, vid, service)
        try:
            assert df.duplicated().sum() == 0
        except:
            raise ValueError(vid)
        df = df.loc[~df.duplicated(['epoch', 'VehicleId'])].reset_index(drop=True)

        # validate columns, create utc and initialize sid
        cok = ['VehicleId', 'epoch', 'longitude', 'latitude', 'longitude_gps', 'latitude_gps']
        ...
        df['utc'] = [datetime.isoformat(datetime.utcfromtimestamp(x)) for x in df['epoch']]
        ...
        tx = np.hstack((np.nan, np.diff(df['epoch'].values)))
        ...
        if nok.sum() > 0:
            ...
            # validate
            assert np.all(np.sort(df['epoch'].values) == df['epoch'].values)
    ...
    # debug
    # vx = vx.toPandas()
    # gps_segmentation_vid(vx.loc[vx.index == 0])
    # gps_segmentation_vid(vx.loc[vids['VehicleId'] == '9100FFFF-48A9-D663-DF82-8E63F0480000'])

def event_count_metrics(spark):
    """
    event counts based on eval table
    """

    # validation
    tables = [x.name for x in spark.catalog.listTables()]
    assert ('eval' in tables) and ('events' in tables)

    # count of all events
    query0 = f"""
        SELECT eval.index, COUNT(*) AS nevents
        FROM eval JOIN events
            ON events.VehicleId = eval.VehicleId
            AND events.epoch BETWEEN eval.t0 AND eval.t1
        GROUP BY eval.index"""
    ...
    query1 = f"""
        SELECT
            ...
            CAST(events.EventTriggerTypeId AS STRING) AS EventTriggerTypeId,
            COUNT(*) AS nevents
        FROM eval JOIN events
            ON events.VehicleId = eval.VehicleId
            AND events.epoch BETWEEN eval.t0 AND eval.t1
        WHERE events.EventTriggerTypeId <> 30
        GROUP BY eval.index, events.EventTriggerTypeId"""
    df1 = spark.sql(query1).groupBy('index').pivot('EventTriggerTypeId').max().toPandas()

    # clean up
    df1.columns = [x if x == 'index' else f'nevents_{x}' for x in df1.columns]
    assert df0.shape[0] >= df1.shape[0]
    df1['index'] = df1['index'].astype('int')
    df = pd.merge(left=df0, right=df1, on='index', how='left')
    df = df.fillna(0)
    ...
    query2 = f"""
        SELECT
            CAST(eval.index AS STRING) AS index,
            CONCAT(CAST(events.EventTriggerTypeId AS STRING), '_', CAST(events.EventTriggerSubTypeId AS STRING)) AS NameSubId,
            COUNT(*) AS nevents
        FROM eval JOIN events
            ON events.VehicleId = eval.VehicleId
            AND events.epoch BETWEEN eval.t0 AND eval.t1
        WHERE events.EventTriggerTypeId = 30
        GROUP BY eval.index, events.EventTriggerTypeId, events.EventTriggerSubTypeId"""
    df2 = spark.sql(query2).groupBy('index').pivot('NameSubId').max().toPandas()

    # clean up
    subs = list(set([39, 51, 52, 53, 57]).intersection([int(x.split('30_')[1]) for x in df2.columns if x != 'index']))
    subs = ['index'] + [f'30_{x}' for x in subs]
    others = df2[[x for x in df2.columns if x not in subs and x != 'index']].sum(axis=1).values
    df2 = df2[subs].copy()
    df2['30_others'] = others
    df2.index = df2.pop('index').astype('int')
    df2.index.name = None
    df2.columns = [f'nevents_{x}' for x in df2.columns]
    df2 = df2.fillna(0)
...
def event_speed_metrics(spark):
    """
    event speed metrics based on eval table
    - relevant events - 12,15,16,18,26,30
    """
    ...
    query0 = f"""
        ...
        FROM eval JOIN events
            ON events.VehicleId = eval.VehicleId
            AND events.epoch BETWEEN eval.t0 AND eval.t1
        WHERE events.EventTriggerTypeId IN (12,15,16,18,26,30)
        GROUP BY eval.index"""
    df0 = spark.sql(query0).toPandas()

    # average speed of individual relevant events
    query1 = f"""
        SELECT
            eval.index,
            events.EventTriggerTypeId,
            AVG(events.SpeedAtTrigger) AS avg_speed_events
        FROM eval JOIN events
            ON events.VehicleId = eval.VehicleId
            AND events.epoch BETWEEN eval.t0 AND eval.t1
        WHERE events.EventTriggerTypeId IN (12,15,16,18,26,30)
        GROUP BY eval.index, events.EventTriggerTypeId"""
    df1 = spark.sql(query1).toPandas().pivot(index='index', columns='EventTriggerTypeId', values='avg_speed_events').reset_index(drop=False)

    # clean up
    df1.columns.name = None
    df1.columns = [f'avg_speed_events_{x}' if x != 'index' else x for x in df1.columns]
    assert df0.shape[0] == df1.shape[0]
...
def behavior_count_metrics(spark):
    """
...
def dce_score_metrics(spark):
    """
    dce collision-score metrics based on eval table
    """

    # validation
    tables = [x.name for x in spark.catalog.listTables()]
    assert ('eval' in tables) and ('dce_scores' in tables)
    ...
    query = f"""
        WITH
            q1 AS (
                ...
                FROM eval JOIN dce_scores
                    ON dce_scores.VehicleId = eval.VehicleId
                    AND dce_scores.epoch BETWEEN eval.t0 AND eval.t1
                WHERE dce_scores.ModelKey = 'collision'
                GROUP BY eval.index),
            q2 AS (
                SELECT
                    eval.index,
                ...
                FROM eval JOIN events
                    ON events.VehicleId = eval.VehicleId
                    AND events.epoch BETWEEN eval.t0 AND eval.t1
                WHERE events.EventTriggerTypeId IN (27,30,31)
                GROUP BY eval.index)
        SELECT
            q1.index,
    ...
    assert pd.isnull(df[['index', 'dce_model_values_sum', 'dce_model_values_avg', 'dce_model_values_max']]).values.sum() == 0
    df.index = df.pop('index')
    df.index.name = None
...
def gps_metrics(spark):
    """
    gps core usage metrics based on eval table
    """

    # validation
    tables = [x.name for x in spark.catalog.listTables()]
    assert ('eval' in tables) and ('gps' in tables)
    ...
    vdg = spark.sql(f"""
        SELECT
            eval.index,
            eval.VehicleId,
            (MAX(gps.epoch) - MIN(gps.epoch)) / (24*60*60) AS days
        FROM gps JOIN eval
            ON gps.VehicleId = eval.VehicleId
            AND gps.epoch BETWEEN eval.t0 AND eval.t1
        GROUP BY eval.index, eval.VehicleId, eval.t0, eval.t1
        ORDER BY eval.index""")
    ...
    segments0 = spark.sql(f"""
        ...
        GROUP BY eval.index, eval.t0, eval.t1, gps.segmentId""")
    ...
    df = spark.sql(f"""
        ...
        FROM vdg
            LEFT JOIN segments
            ON segments.index = vdg.index
            LEFT JOIN motion
            ON motion.index = vdg.index
        ORDER BY vdg.index""").toPandas()
    assert pd.isnull(df['index']).values.sum() == 0
    df.index = df.pop('index')
    df.index.name = None
...
def gps_segmentation_metrics(dcm, spark):
    ...
    dx = dcm[['VehicleId', 'ta', 'tb']].copy()
    dx['t0'] = dx.pop('ta').values.astype('datetime64[s]').astype('int')
    dx['t1'] = dx.pop('tb').values.astype('datetime64[s]').astype('int')
    assert np.unique(dx['t1'] - dx['t0']).size == 1
    days = np.unique(dx['t1'] - dx['t0'])[0] / (60*60*24)
    dx = broadcast(spark.createDataFrame(dx))
    dx.cache()
    dx.createOrReplaceTempView('dx')

    # days between window boundaries and records by vehicle-id / t0 / t1
    s0 = spark.sql(f"""
        SELECT
            dx.VehicleId, dx.t0, dx.t1,
            (MIN(gps.epoch) - dx.t0) / (60*60*24) AS left_window_to_first_segmented_record,
            (dx.t1 - MAX(gps.epoch)) / (60*60*24) AS last_segmented_record_to_right_window,
            COUNT(DISTINCT(gps.segmentId)) AS n_segments
        FROM gps JOIN dx
            ON gps.VehicleId = dx.VehicleId
            AND gps.epoch BETWEEN dx.t0 AND dx.t1
        WHERE gps.segmentId IS NOT NULL
        GROUP BY dx.VehicleId, dx.t0, dx.t1
    ...
    s1 = spark.sql(f"""
        SELECT
            dx.VehicleId, dx.t0, dx.t1, gps.segmentId,
            COUNT(*) AS n_records_segment,
            (MAX(gps.epoch) - MIN(gps.epoch)) / (60*60*24) AS n_days_segment,
            (MIN(gps.epoch) - LAG(MAX(gps.epoch)) OVER(PARTITION BY dx.VehicleId, dx.t0, dx.t1 ORDER BY gps.segmentId)) / (60*60*24) AS n_days_no_segment
        FROM gps JOIN dx
            ON gps.VehicleId = dx.VehicleId
            AND gps.epoch BETWEEN dx.t0 AND dx.t1
        WHERE gps.segmentId IS NOT NULL
        GROUP BY dx.VehicleId, dx.t0, dx.t1, gps.segmentId
    ...
    s2 = spark.sql(f"""
        ...
        FROM s1
        GROUP BY VehicleId, t0, t1
        ORDER BY VehicleId, t0, t1""")
    s2.createOrReplaceTempView('s2')

    # merge s0,s1,s2 and create pandas DataFrame
    ds = spark.sql(f"""
        SELECT
            s0.VehicleId, s0.t0, s0.t1,
        ...
        FROM s0 JOIN s2
            ...
            AND s0.t0 = s2.t0
            AND s0.t1 = s2.t1
        ORDER BY s0.VehicleId, s0.t0, s0.t1""").toPandas()
...
def gps_interval_metrics(spark, datadir, src, dst, service, include_daily_coverage=False):
    ...
    def segment_metrics(df):
        ...
        lon = df['longitude_gps'].values
        lat = df['latitude_gps'].values
        assert (np.isnan(lon).sum() == 0) and (np.isnan(lat).sum() == 0) and (np.isnan(df['epoch']).sum() == 0)
    ...
    def gps_interval_metrics_vid(pdf):
        ...
        if any(~pd.isnull(df['segmentId'])):
            ...
            df = pd.concat((ds, df.loc[pd.isnull(df['segmentId'])]), axis=0).sort_values('epoch').reset_index(drop=True)
        ...
        # all time intervals
        df['all_time_interval_sec'] = np.hstack((np.nan, np.diff(df['epoch'])))
    ...
    # debug
    # vx = vx.toPandas()
    # gps_interval_metrics_vid(vx.loc[vx.index == 0])
...
def get_conn(server):
    ...
    # data-platform
        ...
        access = 'ASIAV54V4OEUFRRN3UXZ'
        secret = 'ItdGZUigj3pcMMPsinozgShzUbGbtw628SGVmvGm'
        session = ''
        return connect(
